/// Output generator for simple symbol log format.
...
impl OutputGenerator for LogOutput {
    fn generate(
        &self,
        symbols: &[(u32, String)],
        file_name: &str,
        ...
        if append_offset != 0 || pointer_offset != 0 || !align_on_append {
            eprintln!("Warning: Append options aren't supported by the \"log\" output parser.");
        }

        // Options
        let mut line_format = String::from("%X: %s");
