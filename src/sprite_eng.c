...
//    {
//#if (LIB_DEBUG != 0)
//        KLog_U2_("Couldn't allocate ", num, " Sprite(s): only ", free - allocStack, " availables !");
//#endif
//
//        return NULL;
//    }
...
//
//#ifdef SPR_DEBUG
//        KLog_U1("allocateSprite: allocating sprite #", *free - spritesBank);
//#endif // SPR_DEBUG
//    }
//
//#ifdef SPR_PROFIL
...
//    // rollback correction
//    if (prof < 0) prof = 100;
//    profil_time[PROFIL_ALLOCATE_SPRITE] += prof;
//#endif // SPR_PROFIL
//
//    return dest;
//}
...
//    {
//#ifdef SPR_DEBUG
//        KLog_U1("releaseSprites: releasing sprite #", *src - spritesBank);
//#endif // SPR_DEBUG
//
//        // release sprite
//        *free++ = *src++;
...
//#endif // SPR_PROFIL
//
//    Sprite** sprites = allocateSprites(dest, num);
//
//    // can't allocate --> return NULL
//    if (!sprites) return NULL;
//
//    u16 attribut = baseAttribut;
//    s16 spriteIndex = baseSpriteIndex;
//    u16 numVDPSprite = spriteDef->maxNumSprite;
//    u16 activeIndex = spriteNum;
//    Sprite** actives = &activeSprites[activeIndex];
//
//    u16 i = num;
//    while(i--)
//    {
...
//        // initialized with specified flags
//        sprite->status = flags & SPR_FLAGS_MASK;
//        sprite->index = activeIndex++;
//
//#ifdef SPR_DEBUG
//        KLog_U1("SPR_addSpritesEx: added a new sprite at ", sprite->index);
//        KLog_U1("  new sprite number = ", activeIndex);
//#endif // SPR_DEBUG
//
//        sprite->visibility = 0;
//        sprite->definition = spriteDef;
...
//        {
//            // allocate VDP sprite
//            spriteIndex = VDP_allocateSprites(numVDPSprite);
//            // not enough --> release sprites and return NULL
//            if (spriteIndex == -1)
//            {
//                // TODO: normally we should release resources for previous sprite in list... resources leak here !
//                releaseSprites(sprites, num);
//                return NULL;
//            }
//
//            // set the VDP Sprite index for this sprite and do attached operation
//            setVDPSpriteIndex(sprite, spriteIndex, numVDPSprite, lastAllocatedVDPSprite);
//
//#ifdef SPR_DEBUG
//            KLog_U3("  allocated ", numVDPSprite, " VDP sprite(s) at ", spriteIndex, ", remaining VDP sprite = ", VDP_getAvailableSprites());
//#endif // SPR_DEBUG
//        }
//        else
//        {
//            // set the VDP Sprite index for this sprite and do attached operation
//            setVDPSpriteIndex(sprite, spriteIndex, numVDPSprite, NULL);
//            // pass to next sprite index
//            spriteIndex += spriteIndexInc;
//        }
//
//        // auto VRAM alloc enabled ?
//        if (flags & SPR_FLAG_AUTO_VRAM_ALLOC)
...
//                // release allocated VDP sprites
//                if (flags & SPR_FLAG_AUTO_SPRITE_ALLOC)
//                    VDP_releaseSprites(sprite->VDPSpriteIndex, spriteDef->maxNumSprite);
//
//                releaseSprites(sprites, num);
//                return NULL;
//            }
...
//            // pass to next vram index
//            attribut += vramIndexInc;
//        }
//
//        // set anim and frame to 0
//        SPR_setAnimAndFrame(sprite, 0, 0);
//    }
//
//    // store new number of sprite
//    spriteNum = activeIndex;
//
//#ifdef SPR_PROFIL
//    prof = getSubTick() - prof;
//    // rollback correction
...
//{
//    return SPR_addSpritesEx(dest, num, spriteDef, x, y, attribut, 0, 0, 0,
//                           SPR_FLAG_AUTO_VISIBILITY | SPR_FLAG_AUTO_VRAM_ALLOC | SPR_FLAG_AUTO_SPRITE_ALLOC | SPR_FLAG_AUTO_TILE_UPLOAD);
//}
...
void SPR_releaseSprite(Sprite *sprite)
{
    ...
    KLog_U2("  last VDP sprite link: ", linkToNext, "   new sprite number = ", spriteNum);
#endif // SPR_DEBUG
...
//#endif // SPR_PROFIL
//
//    // release sprites (no error checking here)
//    releaseSprites(sprites, num);
//
//#ifdef SPR_DEBUG
//    KLog_U1_("SPR_releaseSprites: releasing ", num, " Sprite(s)");
//#endif // SPR_DEBUG
//
//    Sprite** src = sprites;
//    u16 i = num;
...
//        // get index of sprite in active list
//        u16 index = sprite->index;
//        // get link to next sprite
//        u8 linkToNext = sprite->lastVDPSprite->link;
//
//        // update link with previous
//        if (index) activeSprites[index - 1]->lastVDPSprite->link = linkToNext;
//        else starter->link = linkToNext;
//
//        u16 lastSpriteIndex = spriteNum - 1;
//        // get last sprite
//        Sprite* lastSprite = activeSprites[lastSpriteIndex];
...
//
//#ifdef SPR_DEBUG
//        KLog_U2_("  releasing sprite #", sprite - spritesBank, " (index = ", sprite->index, ")");
//        KLog_U2_("  sprite[", lastSpriteIndex, "] moved to sprite[", index, "]");
//        KLog_U2("  last VDP sprite link: ", linkToNext, "   new sprite number = ", spriteNum);
//#endif // SPR_DEBUG
//
...
static void updateSpriteTableAll(Sprite *sprite)
{
    inline void updateSpriteTableAll_allVisible(Sprite *sprite)
    {
        ...
        cache = &vdpSpriteCache[sprite->VDPSpriteIndex];
        ...
        while(j--)
        {
            ...
            cache->y = sprite->y + spriteInf->y;
            cache->size = spriteInf->size;
            cache->attribut = attr;
            cache->x = sprite->x + spriteInf->x;
    ...
    inline void updateSpriteTableAll_allVisibleHFlip(Sprite *sprite)
    {
        ...
        while(j--)
        {
            ...
            cache->size = size;
            cache->attribut = attr;
            cache->x = sprite->x + (fw - (spriteInf->x + (((size & 0x0C) << 1) + 8)));
    ...
    inline void updateSpriteTableAll_allVisibleVFlip(Sprite *sprite)
    {
        ...
        while(j--)
        {
            ...
            // Y first to respect VDP field order
            cache->y = sprite->y + (fh - (spriteInf->y + (((size & 0x03) << 3) + 8)));
            cache->size = size;
            cache->attribut = attr;
            cache->x = sprite->x + spriteInf->x;
...
static void updateSpriteTablePos(Sprite *sprite)
{
    ...
    inline void updateSpriteTablePos_allVisibleHFlip(Sprite *sprite)
    {
        ...
        while(j--)
        {
            ...
            // Y first to respect VDP field order
            cache->y = sprite->y + spriteInf->y;
            cache->x = sprite->x + (fw - (spriteInf->x + (((spriteInf->size & 0x0C) << 1) + 8)));
    ...
    inline void updateSpriteTablePos_allVisibleVFlip(Sprite *sprite)
    {
        ...
        while(j--)
        {
            ...
            // Y first to respect VDP field order
            cache->y = sprite->y + (fh - (spriteInf->y + (((spriteInf->size & 0x03) << 3) + 8)));
            cache->x = sprite->x + spriteInf->x;
