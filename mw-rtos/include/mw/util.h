...
static int urlencode(char *out, const char *in)
{
	int i, j;
	unsigned char c;
...
static void bin_to_str(const uint8_t *bin, unsigned bin_len, char *str)
{
	int i, j;
...
static int ipv4_to_str(uint32_t ipv4, char str[14])
{
...
static const char *StrCpySrc(char *dst, const char *src) {
	while ('\0' != *src) *dst++ = *src++;
	*dst = '\0';
...
static char *StrCpyDst(char *dst, const char *src) {
	while ('\0' != *src) *dst++ = *src++;
	*dst = '\0';
...
static int itemizer(const char *input, const char **item, int max_tokens)
{
	int i;
