...
void MegaDeviceX7::outTransferCallback(usb_transfer_t *transfer)
{
    ...
    if (transfer->status == USB_TRANSFER_STATUS_COMPLETED) {
        ESP_LOGI(MEGA_DEVICE_TAG, "outTransferCallback: BULK OUT transfer completed");
        //ESP_ERROR_CHECK(usb_host_transfer_free(transfer));    
    }    
    MegaDeviceX7 *megaDevice = (MegaDeviceX7 *)transfer->context;
    megaDevice->mw->lsd->LsdTxBufFree();
}

void MegaDeviceX7::inTransferCallback(usb_transfer_t *transfer)
{
    MegaDeviceX7 *megaDevice = (MegaDeviceX7 *)transfer->context;
    ESP_LOGD(MEGA_DEVICE_TAG,"INPUT[%d]:", transfer->actual_num_bytes);
    ESP_LOG_BUFFER_HEX_LEVEL(MEGA_DEVICE_TAG, transfer->data_buffer, transfer->actual_num_bytes, ESP_LOG_DEBUG); 
    megaDevice->mw->lsd->LsdRecv(transfer->data_buffer, transfer->actual_num_bytes);
    if (transfer->status == USB_TRANSFER_STATUS_COMPLETED) { 
