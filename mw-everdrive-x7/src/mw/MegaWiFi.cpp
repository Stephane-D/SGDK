...
size_t MegaWiFi::MwFsm(MwFsmMsg *msg) {
	...
	switch (d.s.sys_stat) {
		...
		case MW_ST_AP_JOIN:
			if (MW_EV_WIFI == msg->e) {
				ap_join_ev_handler((system_event_t *)(msg->d));
		...
		case MW_ST_READY:
			...
			//data_out_len = MwFsmReady(msg, data_out);
...
void MegaWiFi::ap_join_ev_handler(system_event_t *wifi)
{
	ESP_LOGD(MW_TAG,"WiFi event: %d", wifi->event_id);
...
#ifdef _DEBUG_MSGS
	char md5_str[33];
	mbedtls_md5((const unsigned char*)cfg.md5, md5_str);
	ESP_LOGI(MW_TAG,"loaded MD5:   %s", md5_str);
	mbedtls_md5((const unsigned char*)md5, md5_str);
...
esp_err_t MegaWiFi::wifi_init(){
	...
	if (err) {
		ESP_LOGE(MW_TAG,MW_TAG,"wifi init failed: %s", esp_err_to_name(err));
	}else{
        //WIFI_STORAGE_FLASH
...
size_t MegaWiFi::MwFsmCmdProc(MwCmd *c, uint16_t totalLen) {
	...
	switch (ByteSwapWord(c->cmd)) {
		...
		case MW_CMD_HTTP_METHOD_SET:
			if (http_method_set((esp_http_client_method_t)c->data[0])) {
				reply.cmd = htons(MW_CMD_ERROR);
			}
			lsd->LsdSend((uint8_t*)&reply, MW_CMD_HEADLEN, 0);
			break;
...
int MegaWiFi::MwAccept(int sock, int ch) {
	...
	ESP_LOGI(MW_TAG,"Socket %d, channel %d: established connection from %s.",
...
int MegaWiFi::parse_game_request(struct mw_ga_request *req, MwCmd *reply)
{
	...
	status = ga_request((esp_http_client_method_t)req->method, req->num_paths, req->num_kv_pairs,
			req->req, &body_len);
	if (!status) {
		reply->cmd = htons(MW_CMD_ERROR);
		return 0;
