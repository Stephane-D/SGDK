...
//	ESP_ERROR_CHECK(uart_driver_install(UART_NUM_0, 1024, 0, 0, NULL, 0));
	...
	//lsdData.semtx = xSemaphoreCreateCounting(LSD_BUF_FRAMES, LSD_BUF_FRAMES);
	this->q = q;
	this->qTx = qtx;
}
...
size_t LSD::LsdSend(const uint8_t *data, uint16_t len, uint8_t ch) {
	...
	//xSemaphoreTake(lsdData.semtx, portMAX_DELAY);
...
void LSD::LsdTxBufFree(void) {
	// Just increment the receiver semaphore count. Might cause problems
	// if not properly used!
	//xSemaphoreGive(lsdData.semtx);
}
		...
		switch (lsdData.rxs) {
			...
			case LSD_ST_ETX_RECV:		// ETX should come here
				if (LSD_STX_ETX == recv) {
					...
					ESP_LOGD(LSD_TAG,"Send Message to MW Thead");
