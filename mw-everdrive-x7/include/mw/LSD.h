...
public:
	...
	typedef struct {
		...
		MwMsgBuf tx[LSD_BUF_FRAMES];	///< Reception buffers.
	//	SemaphoreHandle_t semtx;		///< Semaphore to control buffers
		uint16_t postx;			///< Position in current buffer
	...
	****************************************************************************/
	...
	/************************************************************************//**
	* Frees the oldest Transfer buffer. This function must be called each time
	* a buffer is processed to allow receiving a new frame.
	*
	* \warning Calling this function more times than buffers have been received,
	* will likely cause overruns!
	****************************************************************************/
	void LsdTxBufFree(void);
