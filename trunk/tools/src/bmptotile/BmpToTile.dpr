...
begin
   ...
   begin
      if (ParamStr(ii) = '-spr') then sprite := true
      else if (FileName = '') then FileName:= UnixPathToDosPath(ParamStr(ii))
   ...
   try
      ...
      begin
         ...
         begin
            ...
            begin
               if (MemInput.Read(jj, formatint) <> 0) then
                  ss := ss + '0x' + IntToHex(jj, 2 * formatint) + ', '
               else
      ...
      end else FileOutput.Add('    dc.' + formatasm + '  ' + '0x' + IntToHex(nullfill, 2 * formatint));
